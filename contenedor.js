...
class Contenedor {
    ...
    //fs crear objeto (title, price, thumbnail, id)y devolver el id asignado
        async save(objUser){
            try{ 
            const data = await fs.promises.readFile("./datos/productos.txt", "utf-8");
            ...
            await fs.promises.writeFile("./datos/productos.txt", stringUsers)
            return users
        }catch{
            console.log("error");
        }
    ...
    async azar(){
        try{ 
            ...
            await fs.promises.writeFile("./datos/Azar.txt", stringAzar)
